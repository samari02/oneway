...
let aoiHostContainer: HTMLDivElement | null = null
 ...
 */
function setHostVisibility(hidden: boolean): void {
  isHidden = hidden
  if (aoiHostContainer) {
...
async function createAoiWidget(): Promise<void> {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    if (!changes[HIDDEN_GLOBAL_KEY] && !changes[HIDDEN_DOMAINS_KEY]) return
    void (async () => {
 ...
 */
function applyBlockOverlay(): void {
  if (document.getElementById('clarity-block-overlay')) return
 ...
 */
function redirectToBlockScreen(reason: string): void {
  ...
  const blockUrl =
    chrome.runtime.getURL('block-screen.html') +
