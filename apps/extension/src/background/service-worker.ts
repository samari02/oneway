...
async function setIsActive(isActive: boolean) {
...
async function getDisableFrictionSecs(): Promise<{ secs: number }> {
  ...
  const secs = (DISABLE_FRICTION_OPTIONS as readonly number[]).includes(n)
    ? n
    : DEFAULT_DISABLE_FRICTION_SECS
  return { secs }
}

async function setDisableFrictionSecs(secs: unknown): Promise<{ secs: number }> {
  const n = Number(secs)
  const next = (DISABLE_FRICTION_OPTIONS as readonly number[]).includes(n)
    ? n
    : DEFAULT_DISABLE_FRICTION_SECS
...
async function getPageAnalysisData(data: { url: string; domain: string }): Promise<{
  ...
  layer1: {
    checked: boolean
    blocked: boolean
    matchedRule: { pattern: string; reason: string; category: string } | null
  }
  ...
  // Current state
  heightenedMode: { active: boolean; expiresAt?: number | null; reason?: string | null }
  thresholds: { warn: number; block: number }
  finalDecision: 'allow' | 'warn' | 'block'
  ...
  try {
    ...
    let matchedRule: { pattern: string; reason: string; category: string } | null = null
    ...
    const getActionFromScore = (score: number, flags: string[]): string => {
    ...
    const checks: { name: string; score: number; detail: string; status: 'pass' | 'warn' | 'fail' }[] = []
    ...
    if (pageAnalysis) {
      ...
      const whitelistReason = reasons.find((r: string) => r.includes('whitelist'))
      checks.push({
      ...
      // Meta tags check
      const metaReason = reasons.find((r: string) => r.toLowerCase().includes('meta') || r.includes('rating'))
      checks.push({
        name: 'Meta Tags',
      ...
      // Title check  
      const titleReason = reasons.find((r: string) => r.toLowerCase().includes('title'))
      checks.push({
        name: 'Page Title',
      ...
      // Body content check
      const bodyReason = reasons.find((r: string) => r.toLowerCase().includes('keyword') && !r.toLowerCase().includes('url') && !r.toLowerCase().includes('title'))
      checks.push({
        name: 'Body Content',
      ...
      // URL path check
      const urlReason = reasons.find((r: string) => r.toLowerCase().includes('url'))
      checks.push({
        name: 'URL Path',
      ...
      // Media ratio check
      const mediaReason = reasons.find((r: string) => r.toLowerCase().includes('media') || r.toLowerCase().includes('ratio'))
      checks.push({
      ...
      // Links check
      const linkReason = reasons.find((r: string) => r.toLowerCase().includes('link'))
      checks.push({
      ...
      // Safe context
      const safeReason = reasons.find((r: string) => r.toLowerCase().includes('safe context'))
