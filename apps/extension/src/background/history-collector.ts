...
import type { HistoryItem, CategorizedVisit, Category, HistoryStats } from '../shared/types'
 ...
 */
export function isDistraction(category: Category): boolean {
...
export async function importHistory(days: number = 30): Promise<CategorizedVisit[]> {
  ...
  try {
    ...
    const newVisits: CategorizedVisit[] = historyItems
    ...
    const existingSignatures = new Set(
      existingHistory.map((v: CategorizedVisit) => `${v.domain}:${v.visitTime}`)
    )
 ...
 */
export function calculateHistoryStats(visits: CategorizedVisit[]): HistoryStats {
  ...
  const domainCounts = new Map<string, { count: number; category: Category }>()
  ...
  for (const visit of visits) {
    ...
    // Track time range
    if (visit.visitTime < minTime) minTime = visit.visitTime
    if (visit.visitTime > maxTime) maxTime = visit.visitTime
  }
...
export async function getCollectionStatus(): Promise<{
  ...
  const hasPermission = await hasHistoryPermission()
  const { navigationHistory = [], historyLastImport, historyPeriodDays } = 
    await chrome.storage.local.get(['navigationHistory', 'historyLastImport', 'historyPeriodDays'])
