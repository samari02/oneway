...
export async function syncHistoryToSupabase() {
    ...
    try {
        ...
        const visitsToSync = navigationHistory.filter(visit => visit.visitTime > lastSupabaseSync);
...
export async function getSyncStatus() {
    ...
    const pendingCount = lastSupabaseSync
        ? navigationHistory.filter(v => v.visitTime > lastSupabaseSync).length
 ...
 */
export async function cleanupSyncedHistory() {
    const { navigationHistory = [], lastSupabaseSync = 0 } = await chrome.storage.local.get([
        'navigationHistory',
        'lastSupabaseSync'
    ]);
    ...
    const recentHistory = navigationHistory.filter(visit => visit.visitTime > cutoff);
 ...
 */
export async function fetchStatsFromSupabase(days = 30) {
    const user = await getCurrentUser();
    if (!user)
        return null;
